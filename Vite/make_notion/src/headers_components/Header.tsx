...
`;
const SideButton = styled.button<{ bedgeCount: number }>`
  flex-shrink: 0;
...
`;
const Cover = styled.div<{ zindex: number }>`
  position: absolute;
  background-color: white;
  width: 100%;
  height: 100%;
  transition:
    ...
    z-index 0.5s ease-in;
  z-index: ${(props) => props.zindex};
  opacity: ${(props) => props.zindex / 10 + 1};
`;
function Headers({
  coverZindex,
  onHoveredMouse,
  ...
  onShow: (newSideSignals: { clicked: boolean; hovered: boolean }) => void;
  bedgeCount: number;
}) {
  ...
  return (
    ...
    >
      ...
      <div className="Header Right">
        <button>공유</button>
        <button>
          <img src={messageSVG} alt="open reply bar" />
        </button>
        <button>
          <img src={clockSVG} alt="all updates" />
        </button>
        <button>
          <img src={starSVG} alt="bookmarking" />
        </button>
        <button>
